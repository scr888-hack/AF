...
maxgie = LINE('EN0zxIK46A8c4P7WXZhd.+6NKLsfBTg3oZKrP7lu/lq.Z57Z/0/x9iQvuLNyS6KKh3w36sx20He5aajRFgnazmc=')
#maxgie = LINE('')
...
lineSettings = maxgie.getSettings()
#==============================================================================#
maxgiePoll = OEPoll(maxgie)
maxgieMID = maxgie.getProfile().mid
responsename = maxgie.getProfile().displayName
KAC = [maxgie]
Bots = [maxgie]
Owner = [maxgieMID]
admin = [maxgieMID]
Exc = [Owner,admin,maxgieMID]
    ...
    try:
        ...
        if op.type == 5:
            ...
            if settings["autoAdd"] == True:
                maxgie.sendMessage(op.param1, "สวัสดี {} ขอบคุณที่แอดมา".format(str(maxgie.getContact(op.param1).displayName)))
#                maxgie.sendMessage(op.param1, str(settings["welcome"]))
                maxgie.sendMessage(op.param1, str(settings["สน"]))
        if op.type == 5:
...
#                maxgie.sendMessage(op.param1, str(settings["welcome"]))
                maxgie.sendMessage(op.param1, str(settings["commet"]))
        if op.type == 13:
...
#==============================================================================#
                ...
                if text.lower() == "เชคค่า" or text.lower() == "set":
                  ...
                  if msg._from in admin:
                    ...
                    start = time.time()
                    maxgie.sendMessage(to, "รอสักครู่...")
                    elapsed_time = time.time() - start
                    maxgie.sendMessage(to,format(str(elapsed_time)))
                elif text.lower() == 'รีบอท':
                  if msg._from in admin and Owner:
                    maxgie.sendMessage(to, "รอสักครู่...")
                    time.sleep(5)
                    maxgie.sendMessage(to, "เรียบร้อย")
                    restartBot()
                elif text.lower() == 'บอทออน':
                    timeNow = time.time()
                    runtime = timeNow - botStart
                    runtime = format_timespan(runtime)
                    maxgie.sendMessage(to, "ระยะเวลาการทำงานของบอท {}".format(str(runtime)))
                    ...
                    if msg._from in Owner:
                        ...
                        for target in targets:
                            try:
                                ...
                                maxgie.sendMessage(msg.to,"☢-Bot-☢\nจดจำ\nเรียบร้อยคับ")
                            except:
                                pass
                    else:
                        maxgie.sendMessage(msg.to,"Owner Permission Required")
...
#-------------------------------------------------------------------------------
                    ...
                    if msg._from in admin:
                        if settings["protect"] == True:
                            if settings["lang"] == "JP":
                                maxgie.sendMessage(msg.to,"➲ Protection Already On")
                            else:
                                maxgie.sendMessage(msg.to,"➲ Protection Set To On")
                        else:
                    ...
                    if msg._from in admin:
                        if settings["protect"] == False:
                            if settings["lang"] == "JP":
                                maxgie.sendMessage(msg.to,"➲ Protection Already Off")
                            else:
                                maxgie.sendMessage(msg.to,"➲ Protection Set To Off")
                        else:
...
#-------------------------------------------------------------------------------
                    ...
                    if msg._from in admin:
                        ...
                        maxgie.sendMessage(msg.to,"➲ All Protect Set To On")
                    else:
                        maxgie.sendMessage(msg.to,"Just for Owner")
                    ...
                    if msg._from in admin:
                        ...
                        maxgie.sendMessage(msg.to,"➲ All Protect Set To Off")
                    else:
                        maxgie.sendMessage(msg.to,"Just for Owner")
...
#-------------------------------------------------------------------------------
                  ...
                  if msg._from in admin:
                    settings['detectMention'] = True
                    maxgie.sendMessage(msg.to,"Respon enabled.")
                elif msg.text in ["เปิดแทค"]:
                  if msg._from in admin:
                  ...
                  if msg._from in admin:
                    settings["Aip"] = True
                    maxgie.sendMessage(msg.to,"เปิดระบบตรวจสอบคำหยาบกับบอทบิน  ^ω^")

                elif msg.text in ["ปิดตรวจสอบคำหยาบ"]:
                  if msg._from in admin:
                    settings["Aip"] = False
                    maxgie.sendMessage(msg.to,"ปิดระบบตรวจสอบคำหยาบกับบอทบินแล้วʕ•ﻌ•ʔ")

                elif msg.text in ["เปิดอ่าน"]:
                  if msg._from in admin:
                    settings["autoRead"] = True
                    maxgie.sendMessage(msg.to,"เปิดระบบอ่านออโต้")

                elif msg.text in ["ปิดอ่าน"]:
                  if msg._from in admin:
                    settings["autoRead"] = False
                    maxgie.sendMessage(msg.to,"ปิดระบบอ่านออโต้แล้ว")
#==============================================================================#
                elif "ปวดตับ" in msg.text:
                     if msg._from in Owner:
                         _name = msg.text.replace("ปวดตับ","")
                         gs = maxgie.getGroup(receiver)
                         maxgie.sendMessage(receiver,"ต้องรีบไปหาหมอ ô")
                         targets = []
                         for g in gs.members:
                             if _name in g.displayName:
                                 targets.append(g.mid)
                         if targets == []:
                             maxgie.sendMessage(receiver,"Not found.")
                         else:
                             for target in targets:
                                 if not target in admin:
                                     try:
                                         maxgie.kickoutFromGroup(receiver,[target])
                                         print ((receiver,[g.mid]))
                                     except:
                                         maxgie.sendMessage(receiver,"ไปก่อนนะ")
                                         print ("คลีนิค ปิดเรียบร้อย")
#==============================================================================#

                elif text.lower() == 'ติ้ก on':
                    settings["checkSticker"] = True
                    maxgie.sendMessage(to, "❥เปิดระบบเช็คสติ้กเกอร์ ❋")
                elif text.lower() == 'ติ้ก off':
                    settings["checkSticker"] = False
                    maxgie.sendMessage(to, "❥ปิดระบบเช็คสติ้กเกอร์ ❋")
                  ...
                  if msg._from in admin:
                    settings["autoBlock"] = True
                    maxgie.sendMessage(to, "เปิดใช้งานออโต้บล็อคแล้ว.")
                elif text.lower() == 'ปิดบล็อค':
                  if msg._from in admin:
                    settings["autoBlock"] = False
                    maxgie.sendMessage(to, "ปิดใช้งานออโต้บล็อคแล้ว")
                elif text.lower() == 'เปิดลิ้งแชร์':
                  if msg._from in admin:
                    settings["timeline"] = True
                    maxgie.sendMessage(to, "เปิดลิ้งแชร์แล้ว.")
                elif text.lower() == 'ปิดลิ้งแชร์':
                  if msg._from in admin:
                    settings["timeline"] = False
                    maxgie.sendMessage(to, "ปิดลิ้งแชร์แล้ว.")
                elif text.lower() == 'เปิดตรวจสอบ':
                  if msg._from in admin:
                    settings["checkContact"] = True
                    maxgie.sendMessage(to, "ตรวจสอบ สมาชิก ถูกเปิดแล้ว")
                elif text.lower() == 'ปิดตอบ':
                  if msg._from in admin:
                    settings["delayMention"] = False
                    maxgie.sendMessage(to, "ตรวจสอบ สมาชิก ถูกปิดแล้ว")
                  ...
                  if msg._from in admin:
                    gruplist = maxgie.getAllContactIds()
                    kontak = maxgie.getContacts(gruplist)
                    num=1
                    msgs="═════════ไอดีเพื่อน═════════"
                    for ids in kontak:
                        msgs+="\n[%i] %s" % (num, ids.mid)
...
#=======================================================================================
                    ...
                    maxgie.sendMessage(msg.to,"➠เชิญเข้าร่วมการโทรสำเร็จ (｡◕‿◕｡) ")
#=======================================================================================
...
#=======================================================================================
                    ...
                    if msg._from in admin:
                        settings["blacklist"] = {}
                        maxgie.sendMessage(msg.to,"ล้างเรียบร้อย")
                    else:
                    ...
                    if msg._from in admin:
                        ...
                        maxgie.sendMessage(msg.to,"Send Contact")
                    else:
                        maxgie.sendMessage(msg.to,"คุณไม่มีอำนาจสั่ง")
...
#-------------------------------------------------------------------------------
                    ...
                    if msg._from in admin:
                        if settings["blacklist"] == {}:
                            maxgie.sendMessage(msg.to,"ไม่พบผู้ติดดำ")
                        else:
                            maxgie.sendMessage(msg.to,"กำลังโหลด......")
                            num=1
                            msgs="══════════List Blacklist═════════"
                            for mi_d in settings["blacklist"]:
                                msgs+="\n[%i] %s" % (num, maxgie.getContact(mi_d).displayName)
                                num=(num+1)
...
#-------------------------------------------------------------------------------
                    ...
                    if msg._from in admin:
                        if settings["mid"] == {}:
                            maxgie.sendMessage(msg.to,"*ไม่มี*")
                        else:
                            maxgie.sendMessage(msg.to,"กำลังโหลด......")
                            num=1
                            msgs="══════════༺ஆืਹໂ√န༻═════════"
                       ...
                       for target in targets:
                           try:
                               ...
                               maxgie.sendMessage(msg.to,"สำเร็จ❋")
                               print ("update User")
                           except:
                               maxgie.sendMessage(msg.to,"ผิดพลาด")
#=======================================================================================
                elif text.lower() == 'owner':
                    if msg._from in admin:
                        if Owner == []:
                            maxgie.sendMessage(msg.to,"The Ownerlist is empty")
                        else:
                            maxgie.sendMessage(msg.to,"กำลังโหลด...")
                            mc = "╔═══════════════\n╠♥ ✿✿✿ 🔥Hack Scan 🔔Win🔔  ✿✿✿ ♥\n╠══✪〘 Owner List 〙✪═══\n"
                            for mi_d in Owner:
                                mc += "╠✪ " +maxgie.getContact(mi_d).displayName + "\n"
                            maxgie.sendMessage(msg.to,mc + "╠═══════════════\n╠✪〘 line.me/ti/p/~vipscanner_win 〙\n╚═══════════════")
                elif 'เพิ่มเพื่อน' in text.lower():
                       targets = []
                       key = eval(msg.contentMetadata["MENTION"])
                       key["MENTIONEES"] [0] ["M"]
                  ...
                  if msg._from in admin:
                    ...
                    for a in range(k+1):
                        ...
                        for i in group.members[a*20 : (a+1)*20]:
                            ...
                            s += 7
                            txt += u'@Alin \n'
                        maxgie.sendMessage(to, text=txt, contentMetadata={u'MENTION': json.dumps({'MENTIONEES':b})}, contentType=0)
                    else:
                        maxgie.sendMessage(to, "ทั้งหมด {}  คน".format(str(len(nama))))          
#=======================================================================================
                if text.lower() == "ดำ":
                  if msg._from in admin:
...
#=======================================================================================
                    ...
                    maxgie.sendMessage(msg.to, str(settings["sms"]))
                elif msg.text in ("ข้อความ2"):
                    maxgie.sendMessage(msg.to, str(settings["sms1"]))
                  ...
                  if msg._from in admin and Owner:
                     ...
                     if spl in [""," ","\n",None]:
                         maxgie.sendMessage(msg.to, "❋ตั้งข้อความเรียบร้อยแล้ว❋")
                          ...
                          settings["Pic"] = spl
                          maxgie.sendImageWithURL(to, "{}".format(str(spl)))
#                          maxgie.sendMessage(msg.to, "เรียบร้อย❋\n\n{}".format(str(spl)))
                elif 'ID: ' in msg.text:
                  if msg._from in admin and Owner:
                  ...
                  if msg._from in admin and Owner:
                     ...
                     if spl in [""," ","\n",None]:
                         maxgie.sendMessage(msg.to, "❋ตั้งข้อความเรียบร้อยแล้ว❋")
                          ...
                          settings["FT"] = spl
                          maxgie.sendMessage(msg.to, "เรียบร้อย❋\n\n{}".format(str(spl)))
                elif 'ข้อความปุ่ม: ' in msg.text:
                  if msg._from in admin and Owner:
                     spl = msg.text.replace('ข้อความปุ่ม: ','')
                     if spl in [""," ","\n",None]:
                         maxgie.sendMessage(msg.to, "❋ตั้งข้อความเรียบร้อยแล้ว❋")
                     else:
                          settings["ปุ่ม"] = spl
...
#                                                "color":"#CC0033",
                ...
                elif msg.text.lower().startswith("#pc "):
                            delcmd = msg.text.split(" ")
                            get = msg.text.replace(delcmd[0]+" ","").split(">")
                            kw = get[0]
                            ans = get[1]
                            ...
                            sendTemplate(to, data)
                elif msg.text.lower().startswith("#gpc "):
                        delcmd = msg.text.split(" ")
                        get = msg.text.replace(delcmd[0]+" ","").split(">")
                        kw = get[0]
                        ans = get[1]
                ...
                elif msg.text.lower().startswith("#1pc "):
                     delcmd = msg.text.split(" ")
                     get = msg.text.replace(delcmd[0]+" ","").split(">")
                     kw = get[0]
                     ans = get[1]
                     ...
                     for i in gid:
                ...
                elif msg.text.lower().startswith("#fpc "):
                        delcmd = msg.text.split(" ")
                        get = msg.text.replace(delcmd[0]+" ","").split(">")
                        kw = get[0]
                        ans = get[1]
                        ...
                        for i in gid:
                            ...
                            sendTemplate(to, data)
                elif text.lower() == ".1gpc":
                    sa = "{}".format(str(settings["Pic"]))
                    sc = "{}".format(str(settings["LineID"]))
                    PT = "{}".format(str(settings["gid"]))
                    groups = maxgie.getGroupIdsJoined()
                    for group in groups:
                        if group in PT:
                            data = {
                ...
                if text.lower() == "help" or text.lower() == "คำสั่ง":
                            ...
                            sendTemplate(to, data)
                if text.lower() == "ดูคำสั่ง" or text.lower() == "settingowner":
                            sv = "{}".format(str(settings["web"]))
                            s = "#FF0000"
                            sa = """                if text.lower() == "เปิดแทค":
                    tagadd["tags"] = True
                    sa = "เปิดแล้วว >_<"
                ...
                if text.lower() == "เปิดโค๊ดติ๊ก":
                    sets["checkSticker"] = True
                    maxgie.sendMessage(to,"เปิดแล้ว >_<")
                if text.lower() == "ปิดโค๊ดติ๊ก":
                    sets["checkSticker"] = False
                    maxgie.sendMessage(to,"ปิดแล้ว >_<")
                ...
                if text.lower() == "stic off":
                    ...
                    maxgie.sendMessage(to,"ปิดแล้ว >_<")"""
                            ...
                            dataProfile = [
                                {
                                    ...
                                    "header": {
                                        ...
                                        "contents": [
                                            {
                                                "type": "image",
                                                "url": 'https://www.img.in.th/images/acaf2cf7699fe1b13bf3237d7ad39cad.gif',
                                                "size": "full"
                                            },
                                            {
                                                "type": "text",
                            ...
                            sendTemplate(to, data)
                if text.lower() == "///ด่วน" or text.lower() == "ประกาศทั่วไป":
                            ss = "#FFFF00"
                            s = "#FF0000"
                            sa = "➰..168GALAXY ( PG )"
                            sa += "\nโบนัส30% สูงสุด300*2เทิร์น"
                            sq = "\n\n➰ั..HAPPYSLOT88 ( PG )"
                            sq += "\nโบนัส40% สูงสุด400*1เทิรน"
                            se = "\n\n➰ั..ADMIN1PG ( PG )"
                            se += "\nโบนัส30% สูงสุด300*2เทิร์น"
                            sr = "\n\n➰ั..SLOTXOTH ( PG SLOT )"
                            sr += "\nโบนัส30% สูงสุด300*2เทิร์น"
                            st = "\n\n➰ััั..SLOTXD ( PG ) "
                            st += "\nโบนัส30% สูงสุด300*2เทิรน"
                            sy = "\n\n➰ััั..LUCKYDUB ( PG )"
                            sy += "\nโบนัส50% สูงสุด300*2เทิร์น"
                            su = "\n\n➰ััั..LIVE22EASY ( PG )"
                            su += "\nโบนัส30% สูงสุด300*2เทิร์น"
                            dataProfile = [
                                {
                                    ...
                                    "styles": {
                                        "header": {"backgroundColor":"#000000"},
                                        "hero": {"backgroundColor": "#333300"}, #"separator": True, "separatorColor": "#333333"},
                                        "footer": {"backgroundColor": "#3333CC"}, #"separator": True, "separatorColor": "#333333"},
                                    },
                                    "header": {
                                        "type": "box",
                                        "layout": "vertical",
                                        ...
                                        "contents": [
                                            {
                                                "type": "image",
                                                "url": 'https://www.img.in.th/images/0de19acb893f0f6047c3c43eb74277ca.jpg',
                                                "size": "full"
                                            },
                                            {
                                                "type": "text",
                                        ...
                                        #        "size": "md"
                                            ...
                                            },
                                            {
                                                ...
                                                "action":{
                                                    "type":"uri",
                                                    "label":"สมัครคลิก",
                                                    "uri":"https://ambbo.co/register?prefix=MTY4UEc&ref=5e22b0dbfa8c9b3564b0e83c"
                                                 },
                                            },
                                            {
                                                "type": "text",
                                            ...
                                            },
                                            {
                                                ...
                                                "action":{
                                                    "type":"uri",
                                                    "label":"สมัครคลิก",
                                                    "uri":"https://ambbo.co/register?prefix=UEE&ref=5e21e4d97a74be0a65aaf720"
                                                 },
                                            },
                                            {
                                                "type": "text",
                                            ...
                                            },
                                            {
                                                ...
                                                "action":{
                                                    "type":"uri",
                                                    "label":"สมัครคลิก",
                                                    "uri":"https://ambbo.co/register?prefix=UUFB&ref=5e1a8ee09cc0b946093ffc38"
                                                 },
                                            },
                                            {
                                                "type": "text",
                                            ...
                                            },
                                            {
                                                ...
                                                "action":{
                                                    "type":"uri",
                                                    "label":"สมัครคลิก",
                                                    "uri":"https://ambbo.co/register?prefix=UEdUSA&ref=5e042654bbf5de06434f65d4"
                                                 },
                                            },
                                        ]
                                    },
                                },
                                {
                                    ...
                                    "header": {
                                        ...
                                        "contents": [
                                            {
                                                "type": "image",
                                                "url": 'https://www.img.in.th/images/d2bacd44b8c4452b30b25dd715e2048b.jpg',
                                                "size": "full"
                                            },
                                            {
                                                "type": "text",
                                            ...
                                            },
                                            {
                                                ...
                                                "action":{
                                                    "type":"uri",
                                                    "label":"สมัครคลิก",
                                                    "uri":"https://ambbo.co/register?prefix=WEc&ref=5e281642cb923514b6e59f30"
                                                 },
                                            },
                                            {
                                                "type": "text",
                                            ...
                                            },
                                            {
                                                ...
                                                "action":{
                                                    "type":"uri",
                                                    "label":"สมัครคลิก",
                                                    "uri":"https://ambbo.co/register?prefix=TERQ&ref=5e22e54d4ae2ec2a2ef69b0a"
                                                 },
                                            },
                                            {
                                                "type": "text",
                                            ...
                                            },
                                            {
                                                ...
                                                "action":{
                                                    "type":"uri",
                                                    "label":"สมัครคลิก",
                                                    "uri":"https://ambbo.co/register?prefix=TFBH&ref=5e22b88982a53b3bac98a717"
                                                 },
                                            },
                                        ]
                                    },
                            ...
                            data = {
                                "type": "flex",
                                "altText": "⚜️⚜️ มีผู้กล่าวถึงคุณ ⚜️⚜️",
                                "contents": {
                                    "type": "carousel",
                                    "contents": dataProfile
                                }
                            ...
                            sendTemplate(to, data)
                if text.lower() == "///ด่วน" or text.lower() == "ประกาศแชท":
                    ...
                    for i in gid:
                            ...
                            sendTemplate(i, data)
                if text.lower() == "///ด่วน" or text.lower() == "ประกาศกลุ่ม":
                    gid = ["c50acae68fbefc37ba11ea46f522eed9b","c6520f4888b6433cbed86fe747fd1e11d"]
                    groups = maxgie.group
                    for group in groups:
                        if not group in gid:
                            ss = "#FFFF00"
                            ...
                            sendTemplate(group, data)
                if text.lower() == "ราคาสแกน" or text.lower() == "ราคา":
                            s = "#FFFF00"
                            sa = """[-โปรแกรม Scan เกมส์ต่างๆเชิญคับ-]

1. NEW โปรแกรม 918 Scan Win (V.3.1.5) 
    อัพเดตเกมส์ใหม่เข้าแล้วทุกเกมส์
     = ใช้งานถาวร ราคา 300 บาท

2. โปรแกรม Scan Jocker123
     = ใช้งานถาวร ราคา 300 บาท

3. Pussy888 Scan ใช้งานถาวร
     = ใช้งานถาวร ราคา 300 บาท

4. โปรแกรม Scan SlotXo
     = ใช้งานถาวร ราคา 300 บาท

สนใจสอบถาม
Line ID:vipscanner_win\n"""
                            sa += "หรือคลิกที่ปุ่มด้านล่างได้เลย"
                            dataProfile = [
                                {
                                    "type": "bubble",
                                    "styles": {
                                    ...
                                    "header": {
                                        ...
                                        "contents": [
                                            {
                                                "type": "image",
                                                "url": 'https://data.bloggif.com/distant/user/store/d/8/c/f/95e7519fe26f975b5a027a3f0da5fc8d.gif',
                                                "size": "full"
                                            },
                                            {
                                                "type": "text",
                    ...
                    if msg_id in msg_dict:
                        if msg_dict[msg_id]["from"]:
                           if msg_dict[msg_id]["text"] == 'Gambarnya dibawah':
                                ginfo = maxgie.getGroup(at)
                                ryan = maxgie.getContact(msg_dict[msg_id]["from"])
                                zx = ""
                                zxc = ""
                                zx2 = []
                                ...
                                ret_ += "\n⟗   ⃢🕸DK-Bot  ⟗"
                                ret_ += "\nCreator:  line.me/ti/p/~vipscanner_win" 
                                ...
                                maxgie.sendMessage(at, text, contentMetadata={'MENTION':str('{"MENTIONEES":'+json.dumps(zx2).replace(' ','')+'}')}, contentType=0)
                                maxgie.sendImage(at, msg_dict[msg_id]["data"])
                           else:
                                ginfo = maxgie.getGroup(at)
                                ryan = maxgie.getContact(msg_dict[msg_id]["from"])
                                ret_ =  "「 ᴘᴇsᴀɴ ᴅɪʜᴀᴘᴜs  」\n"
                                ...
                                ret_ += "\n 「🔑」Tim DK-BOT🕸 ⃢   ⟗"
                                ret_ += "\nCreator:  line.me/ti/p/~vipscanner_win" 
                                maxgie.sendMessage(at, str(ret_))
                        del msg_dict[msg_id]
                except Exception as e:
                    print(e)
                ...
                try:
                    ...
                    if msg_id in msg_dict1:
                        if msg_dict1[msg_id]["from"]:
                                ginfo = maxgie.getGroup(at)
                                ryan = maxgie.getContact(msg_dict1[msg_id]["from"])
                                ...
                                maxgie.sendMessage(at, str(ret_))
                                maxgie.sendImage(at, msg_dict1[msg_id]["data"])
